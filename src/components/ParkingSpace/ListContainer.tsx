...
const ListContainer: React.FC<ContainerProps> = ({parkingType, loaddata, inputs }) => {
    ...
    if (inputs.length > 0) {
      ...
      <IonIcon slot="end" icon={checkmarkCircle} />
        ...
        <IonModal backdropDismiss={false} isOpen={showModal2} animated={true}  >
          ...
          <IonFab vertical="bottom" horizontal="start" slot="fixed">
            <IonFabButton onClick={() => setShowModal2(false)} routerLink="/home"><IonIcon icon={arrowBackOutline} /></IonFabButton>
          </IonFab>
        </IonModal></>)
    }
  } catch (e) {
